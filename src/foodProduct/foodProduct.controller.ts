...
export class FoodProductController {
  ...
  getFoodProducts(): Promise<FoodProduct[]> {
    this.logger.log('[GET] FoodProduct: getting all FoodProducts');
    return this.foodProductService.findAll();
  }

  @Post()
  createFoodProducts(@Body() foodProducts: CreateFoodProductDto): Promise<FoodProduct[] | null> {
    this.logger.log(`[POST] FoodProduct: ${foodProducts} created`);
  ...
  computeAndSaveAllCarbonFootprint(): Promise<number> {
    this.logger.log('[POST] FoodProduct: computeAndSaveAllCarbonFootprint');
    return this.foodProductService.computeAndSaveAllCarbonFootprint();
  }

  @Post('compute-carbon-footprint')
  @HttpCode(200)
  async computeCarbonFootprint(@Body() computeFoodProductCarbonFootprintDto: ComputeFoodProductCarbonFootprintDto): Promise<{
    carbonFootprint: number;
  }> {
    this.logger.log('[POST] Footprint calculated');
    return {
