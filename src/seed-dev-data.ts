...
export const getTestEmissionFactor = (name: string) => {
  const emissionFactor = TEST_CARBON_EMISSION_FACTORS.find((ef) => ef.name === name);
  if (!emissionFactor) {
    throw new Error(`test emission factor with name ${name} could not be found`);
  }
  return emissionFactor;
};

export const getTestFoodProduct = (name: string) => {
  const foodProduct = TEST_FOOD_PRODUCT.find((ef) => ef.name === name);
  if (!foodProduct) {
...
export const seedTestCarbonEmissionFactors = async () => {
  ...
  const carbonEmissionFactorsService = dataSource.getRepository(CarbonEmissionFactor);
...
export const seedTestFoodProducts = async () => {
  ...
  const service = dataSource.getRepository(FoodProduct);

  await service.save(TEST_FOOD_PRODUCT);
};

if (require.main === module) {
  seedTestCarbonEmissionFactors().catch((e) => Logger.error(e));
  seedTestFoodProducts().catch((e) => Logger.error(e));
}
