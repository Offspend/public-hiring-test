...
import {getTestEmissionFactor, getTestFoodProduct, TEST_CARBON_EMISSION_FACTORS} from "../src/seed-dev-data";
...
describe("FoodProductController", () => {
  ...
  beforeEach(async () => {
    ...
    await dataSource
      ...
      .save([getTestFoodProduct('vegetarian pizza'), getTestFoodProduct('ham pizza')]);

    defaultFoodProduct = await dataSource
            .getRepository(FoodProduct)
            .find();
  });
  ...
  it("POST /food-products/compute-and-save-all", async () => {
    return request(app.getHttpServer())
