...
describe('CarbonEmissionFactorsController', () => {
  ...
  beforeEach(async () => {
    ...
    await dataSource.getRepository(CarbonEmissionFactor).save([getTestEmissionFactor('ham'), getTestEmissionFactor('beef')]);

    defaultCarbonEmissionFactors = await dataSource.getRepository(CarbonEmissionFactor).find();
  });

  it('GET /carbon-emission-factors', async () => request(app.getHttpServer())
